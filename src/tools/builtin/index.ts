 ...
 */
export function getBuiltinToolClass(
  toolName: string
): (new (config?: BuiltinToolConfig) => BuiltinToolBase) | undefined {
  return BUILTIN_TOOLS_REGISTRY[toolName as BuiltinToolName];
}
 ...
 */
export function getAvailableBuiltinTools(): string[] {
 ...
 */
export function getBuiltinToolDefinition(toolName: string, config?: BuiltinToolConfig) {
  const tool = createBuiltinTool(toolName, config);
  if (!tool) {
    return undefined;
  }
