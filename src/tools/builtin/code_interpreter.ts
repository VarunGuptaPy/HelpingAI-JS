...
export class CodeInterpreterTool extends BuiltinToolBase {
  ...
  readonly description =
    'Execute Python code in a secure sandboxed environment with support for data analysis, visualization, and computation. Includes popular libraries like matplotlib, pandas, numpy, and automatic plot saving.';
   ...
   */
  private async executeCodeSimple(code: string): Promise<string> {
   ...
   */
  private runPythonScript(scriptFile: string): Promise<{ stdout: string; stderr: string }> {
...
`;
   ...
   */
  private collectGeneratedImages(): string[] {
    ...
    return new Promise((resolve, reject) => {
      ...
      pythonProcess.on('close', code => {
        if (code === 0) {
          ...
          reject(
            new Error(
              'Python3 is not available on this system. Please install Python 3.x to use the code interpreter.'
            )
          );
        }
      });
