...
class MCPManager {
    ...
    async executeTool(toolName, args) {
        if (!this.initialized) {
            throw new errors_1.MCPError('MCP manager not initialized');
        }
        try {
            const result = await this.client.callTool(toolName, args);
            // Convert MCP result to simple string/object format
            if (result.isError) {
                throw new errors_1.MCPError(`MCP tool execution failed: ${JSON.stringify(result.content)}`);
            }
            ...
            throw new errors_1.MCPError(`Failed to execute MCP tool ${toolName}: ${error instanceof Error ? error.message : String(error)}`);
        }
    }
    /**
