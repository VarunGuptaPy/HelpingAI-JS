...
export class MCPClient {
    ...
    async connect(serverName, config) {
        try {
            ...
            if (!this.isMCPAvailable()) {
                throw new MCPError('MCP SDK not available. Install with: npm install @modelcontextprotocol/sdk', serverName);
            }
            const connection = {
                serverName,
                config,
            ...
            throw new MCPError(`Failed to connect to MCP server ${serverName}: ${error instanceof Error ? error.message : String(error)}`, serverName);
        }
    }
    /**
     * Check if MCP SDK is available
    ...
    async callTool(toolName, args) {
        ...
        try {
            ...
            throw new MCPError(`Failed to call MCP tool ${toolName}: ${error instanceof Error ? error.message : String(error)}`, targetConnection.serverName);
        }
    }
    /**
    ...
    async disconnect(serverName) {
        ...
        try {
            ...
            throw new MCPError(`Failed to disconnect from MCP server ${serverName}: ${error instanceof Error ? error.message : String(error)}`, serverName);
        }
    }
    /**
